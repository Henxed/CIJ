    ...
    <main class="content">
      ...
      <div class="doc">
          <form class="" action="index.html" method="post">
            ...
            <button type="button" class="btn" v-on:click="updatePost">Изменить</button>
          </form>
      </div>
  ...
  methods: {
    ...
    updatePost: function (e) {
      if (this.post.title && this.post.contacts && this.post.boss) {
        store.posts.find({ id: parseInt(this.$route.params.id) }).assign(this.post).write()
        this.ok()
        this.errors = []
        return true;
      }
